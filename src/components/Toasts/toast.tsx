...
//   }
// );
function getToastClassName(variant: "default" | "destructive" = "default") {
  // ベースクラス名
  const baseClassName = [
    "group pointer-events-auto relative flex w-full items-center justify-between space-x-4",
    "overflow-hidden rounded-md border border-zinc-200 p-6 pr-8 shadow-lg transition-all",
...
>(({ className, ...props }, ref) => {
  const baseClasses =
    "inline-flex h-8 shrink-0 items-center justify-center rounded-md border bg-transparent px-3 text-sm font-medium ring-offset-white transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2 disabled:pointer-events-none disabled:opacity-50";

  const lightModeClasses =
    "border-zinc-200 hover:bg-zinc-100 focus:ring-zinc-950";

  const darkModeClasses =
    "dark:border-zinc-800 dark:ring-offset-zinc-950 dark:hover:bg-zinc-800 dark:focus:ring-zinc-300";

  const destructiveClasses =
    "group-[.destructive]:border-zinc-100/40 group-[.destructive]:hover:border-red-500/30 group-[.destructive]:hover:bg-red-500 group-[.destructive]:hover:text-zinc-50 group-[.destructive]:focus:ring-red-500 dark:group-[.destructive]:border-zinc-800/40 dark:group-[.destructive]:hover:border-red-900/30 dark:group-[.destructive]:hover:bg-red-900 dark:group-[.destructive]:hover:text-zinc-50 dark:group-[.destructive]:focus:ring-red-900";
  ...
  return (
    <ToastPrimitives.Action ref={ref} className={combinedClasses} {...props} />
  );
});
...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Close
    ref={ref}
    className={`absolute right-2 top-2 rounded-md p-1 text-zinc-950/50 opacity-0 transition-opacity hover:text-zinc-950 focus:opacity-100 focus:outline-none focus:ring-2 ${
      className || ""
    } group-hover:opacity-100 group-[.destructive]:text-red-300 group-[.destructive]:hover:text-red-50 group-[.destructive]:focus:ring-red-400 group-[.destructive]:focus:ring-offset-red-600 dark:text-zinc-50/50 dark:hover:text-zinc-50`}
    toast-close=""
    {...props}
  >
    <X className="h-4 w-4" />
